...
async function seedDatabase() {
  ...
  // No top-level seed call, moved to startServer
}
...
app.post("/api/auth/register", async (req, res) => {
  ...
  try {
    ...
    const existingUser = db.prepare("SELECT * FROM users WHERE whatsapp = ?").get(whatsapp) as any;
    ...
    if (existingUser) {
      ...
      db.prepare("UPDATE users SET nom = ?, quartier = ?, email = ?, password_hash = ?, otp_code = ?, otp_expiration = ? WHERE id = ?")
        .run(nom, quartier, email, password_hash, otp_code, otp_expiration, existingUser.id);
...
app.post("/api/orders", authenticateToken, (req: any, res) => {
  ...
  try {
    ...
    const user = db.prepare("SELECT nom, whatsapp, quartier, email FROM users WHERE id = ?").get(req.user.id) as any;
    ...
    for (const admin of admins) {
      notifStmt.run(admin.id, `Nouvelle commande #${orderId} de ${user.nom}`, 'order');
    }
...
app.post("/api/reservations", authenticateToken, (req: any, res) => {
  ...
  try {
    ...
    const notifStmt = db.prepare("INSERT INTO notifications (user_id, message, type) VALUES (?, ?, ?)");
    for (const admin of admins) {
      notifStmt.run(admin.id, `Nouvelle réservation de ${req.user.nom} pour ${product_name}`, 'reservation');
    }
...
app.patch("/api/admin/products/:id", authenticateToken, isAdmin, (req, res) => {
  ...
  db.prepare("UPDATE products SET name = ?, price = ?, description = ?, image_url = ? WHERE id = ?")
    .run(name, price, description, image_url, req.params.id);
